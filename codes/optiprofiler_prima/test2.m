function scores = test2()
%TEST2 benchmarks PRIMA double precision against PRIMA quadruple precision.
%
% This test uses problem types 'ubln', dimensions 2--20, and runs the
% 'plain' and 'noisy' features separately.

    fprintf('\nThis test benchmarks PRIMA with double and quadruple precision on plain and noisy problems.\n');
    pause(1.5);
    fprintf('\nStart Test 2...\n\n');

    add_prima_path();

    solvers = {@prima_double, @prima_quadruple};

    base_options.solver_names = {'prima_double', 'prima_quadruple'};
    ...
    merge_test2_summary(fullfile(pwd, base_options.benchmark_id), feature_names);
end
...
function x = prima_quadruple(varargin)
    x = prima_with_precision('quadruple', varargin{:});
end
...
function x = prima_with_precision(precision, varargin)
    ...
    switch numel(varargin)
        ...
        otherwise
            error('test2:InvalidSolverInput', ...
                'Unexpected number of solver inputs: %d.', numel(varargin));
    end
end

function merge_test2_summary(path_out, feature_names)
    ...
    try
        ...
        warning('test2:SummaryMergeFailed', ...
